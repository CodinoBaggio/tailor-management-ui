...
type Props = {
  ...
  deleteDataApi: (seqs: any) => Promise<AxiosResponse<any, any>>;
};
...
export const Maintenance: FC<Props> = (props) => {
  ...
  const handleDelete = () => {
    confirmYesNo('削除しますか？', async () => {
      try {
        const selectedRows = items.filter((item) => rowSelectionModel.includes(item.id));
        const params = selectedRows.map(x => x.seq)
        const res: any = await deleteDataApi(params);
        if (res.status === 'success') {
          getData();
          setRowSelectionModel([]); // チェックボックスをクリア
        } else {
          showMessage('エラー', 'error', res.message);
        }
      } catch (error: any) {
