...
export const FabricProduct = () => {
  ...
  const handleFileRead = (binaryStr: any) => {
    readString(binaryStr, {
      ...
      complete: async (results: any) => {
        ...
        if (results.data[0][0] !== '生地品番' || results.data[0][1] !== '削除フラグ' || results.data[0].length !== 2) {
          showMessage('エラー', 'error', `ヘッダ行を「生地品番,削除フラグ」にしてください`);
          return false;
        }
        ...
        try {
          ...
          const res: any = await masterApi.upsertFabricProductNos({
            userId: user.userId,
  ...
  const handleDownload = async () => {
    setOpen(true);
    try {
      const res: any = await masterApi.getAllFabricProductNosForDownload();
      if (res.status === 'success') {
  ...
  const handleSearch = async () => {
    setOpen(true);
    try {
      const res: any = await masterApi.getFabricProductNosByCondition({ searchPattern: fabricProductNo });
      if (res.status === 'success') {
        setItems(res.payload.productNos);
      } else {
        showMessage('エラー', 'error', res.message);
