...
export const FabricPrice = () => {
  ...
  const getData = useCallback(async () => {
    setOpen(true);
    try {
      const res: any = await masterApi.getAllFabricPrice();
  ...
  const handleSearch = async () => {
    setOpen(true);
    try {
      const res: any = await masterApi.getFabricPriceByCondition({ searchPattern: fabricProductNo });
      if (res.status === 'success') {
        setItems(res.payload.prices);
      } else {
        showMessage('エラー', 'error', res.message);
  ...
  const handleFileRead = (binaryStr: any) => {
    readString(binaryStr, {
      ...
      complete: async (results: any) => {
        ...
        if (
          results.data[0][0] !== '顧客カテゴリ' ||
          results.data[0][1] !== '仲間分け' ||
          results.data[0][2] !== '生地品番' ||
          results.data[0][3] !== '品名' ||
          results.data[0][4] !== '生地広さ' ||
          results.data[0][5] !== '販売価格' ||
          results.data[0][6] !== '原価' ||
          results.data[0][7] !== '削除フラグ' ||
          results.data[0].length !== 8
        ) {
          showMessage(
            'エラー',
            'error',
            `ヘッダ行の形式を「顧客カテゴリ,仲間分け,生地品番,品名,生地広さ,販売価格,原価,削除フラグ」にしてください`
          );
          return false;
        }
        ...
        try {
          ...
          const res: any = await masterApi.upsertFabricPrice({ items: results.data.slice(1) });
          if (res.status === 'success') {
            getData();
            showMessage('登録しました');
          } else {
  ...
  const handleDownload = async () => {
    setOpen(true);
    try {
      const res: any = await masterApi.getAllFabricPriceForDownload();
      if (res.status === 'success') {
