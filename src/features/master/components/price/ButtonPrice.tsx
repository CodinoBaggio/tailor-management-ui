...
export const ButtonPrice = () => {
  ...
  const getData = useCallback(async () => {
    setOpen(true);
    try {
      const res: any = await masterApi.getAllButtonPrice();
  ...
  const handleSearch = async () => {
    setOpen(true);
    try {
      const res: any = await masterApi.getButtonPriceByCondition({ searchPattern: fabricProductNo });
      if (res.status === 'success') {
        setItems(res.payload.prices);
      } else {
        showMessage('エラー', 'error', res.message);
      ...
      complete: async (results: any) => {
        ...
        if (
          ...
          showMessage(
            'エラー',
            'error',
            `ヘッダ行の形式を「仲間分け,品名,ボタン品番,販売価格,原価,削除フラグ」にしてください`
          );
          return false;
        }
        ...
        try {
          ...
          const res: any = await masterApi.upsertButtonPrice({ items: results.data.slice(1) });
          if (res.status === 'success') {
            getData();
            showMessage('登録しました');
          } else {
  ...
  const handleDownload = async () => {
    setOpen(true);
    try {
      const res: any = await masterApi.getAllButtonPriceForDownload();
      if (res.status === 'success') {
