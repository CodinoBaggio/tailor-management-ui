...
export const LiningPrice = () => {
  return (
    <Maintenance
      ...
      columns={columns}
      getDataApi={masterApi.getAllLiningPrice}
      getDataApiForDownload={masterApi.getAllLiningPriceForDownload}
      downloadFileName="裏地品番価格.csv"
      getDataApiByCondition={(productNo: string) => masterApi.getLiningPriceByCondition({ searchPattern: productNo })}
      upsertApi={(params: { data: object[][] }) =>
        masterApi.upsertLiningPrice({
          items: params.data,
        })
      }
      validateHeader={(header: string[]) => {
        if (
          ...
          return 'ヘッダ行の形式を「仲間分け,品名,裏地品番,販売価格,原価,削除フラグ」にしてください';
        }
        return null;
      }}
